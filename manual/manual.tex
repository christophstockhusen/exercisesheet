...
\def\documentoption#1#2{%
  \begin{list}{}{\rightmargin=2em\leftmargin=2em\listparindent=\parindent\parsep=\parskip}%
  \item
  \hskip-\leftmargin\lstinline{#1}\ifempty{#2}{}{\qquad
    default:~\lstinline{#2}}\par\smallskip\noindent\ignorespaces
}
\def\enddocumentoption{\end{list}}

\let\sheetmacro=\documentoption
\let\endsheetmacro=\enddocumentoption
...
\lstinline{narrowfactor} option when loading the document class. The
additional margin then equals 
\begin{equation*}
  (1 - \text{narrow factor}) \cdot \text{text width}.
\end{equation*}
Hence, \lstinline{narrowfactor} takes arguments from $0$ to $1$, the
default value is~$.7$. The option \lstinline{nonarrow} removes any
additional margin.
...
that is \lstinline{student} or \lstinline{instructor}, the titles of 
of the title page and the titles of the exercise sheets change. To
customize the titles, redefine the following macros.

\bigskip

\noindent\emph{To be continued\ldots}
...
The following list contains all the document class options that are
currently available. Every option works with a key-value mechanism,
that is, every key may be called like in the following example.
\begin{lstlisting}
\documentclass[
  narrowfactor=.7,
  solution=true
...
\begin{documentoption}{instructor}{false}
  This option deactivates the dynamic page layout and adjusts the
  titles.
\end{documentoption}

\begin{documentoption}{narrow}{true}
  The \lstinline{narrow} option activates the additional margin on the
...
\begin{documentoption}{nonarrow}{false}
  The \lstinline{nonarrow} option removes the additional margin on the
  right hand side of the page that is intended for correction
  proofreaders marks. 
\end{documentoption}

\begin{documentoption}{nosolution}{true}
  The \lstinline{nosolution} option turns off the typesetting of
  solutions that are typed within the \lstinline{solution}
  environment.
\end{documentoption}

\begin{documentoption}{solution}{true}
  The \lstinline{solution} option turns on the typesetting of
  solutions that are typed within the \lstinline{solution}
  environment.
\end{documentoption}

\begin{documentoption}{student}{true}
